...
class Events
{
   ...
   public static function onMessage($client_id, $message){
        ...
        //";
        file_put_contents("tcpt.txt",$message);
        ...
        if($r != 'error'){
            ...
            if(empty($beltlineinfo)){
                ...
                //
                $t = Date('Ym',time());
                $rowdata = self::$db->select('*')->from('beltline')->where("mn='".$data['mn']."' and no='".$no."'")->row();
                unset($rowdata['id']);
                unset($rowdata['updateTime']);
                self::$db->insert('beltline'.$t)->cols($rowdata)->query();
